...
namespace QuanLyBanHang.Class
{
    class Functions
    {
        ...
        public static void Connect()
        {
            ...
            Con.ConnectionString = @"Data Source=DESKTOP-D884SDQ\SQLEXPRESS; Initial Catalog=Quanlybanhang;User Id=sa; Password=sa"; ;
            Con.Open();                  //Mở kết nối
            //Kiểm tra kết nối
            ...
            for (int i = 0; i <= mLen; i++)
            {
                ...
                if (mLen == i) // Chữ số cuối cùng không cần xét tiếp break; 
                    switch ((mLen - i) % 9)
                    {
                        case 0:
                            mTemp = mTemp + " tỷ";
                            if (i + 1 + 3 <= sNumber.Length && sNumber.Substring(i + 1, 3) == "000") i = i + 3;
                            if (i + 1 + 3 <= sNumber.Length && sNumber.Substring(i + 1, 3) == "000") i = i + 3;
                            if (i + 1 + 3 <= sNumber.Length && sNumber.Substring(i + 1, 3) == "000") i = i + 3;
                            break;
                        case 6:
